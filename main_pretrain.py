...
class Exp:
    def __init__(self, args):
        ...
        elif self.args.method == 'SimVP_pretrain':
            _tmp_input = torch.ones(1, self.args.pre_seq_length, C, H, W).to(self.device)
            flops = FlopCountAnalysis(self.method.model, _tmp_input )
    ...
    def train(self):
        ...
        for epoch in range(self.config['epoch']):
            ...
            if self.args.method in ['SimVP', 'CrevNet', 'PhyDNet','FFT','SimVP_pretrain']:
    ...
    def test(self):
        #best_model_path = '/data/cjj/Documents/zch_Documents/ConvNextVP/results/human_pretrain/checkpoints/90.pth'
        best_model_path = osp.join(self.path, 'checkpoint.pth')
        self.method.model.load_state_dict(torch.load(best_model_path))
        inputs, trues, preds = self.method.test_one_epoch(self.test_loader)
        mae, mse, ssim, psnr = metric(preds, trues, 0, 1, return_ssim_psnr=True)
