...
class ConvGRUCell(nn.Module):
    ...
    def forward(self, x_t, h_t):
        x_concat = self.conv_x(x_t)
        h_concat = self.conv_h(h_t)
        z_x, r_x, o_x = torch.split(x_concat, self.num_hidden, dim=1)
        z_h, r_h, h_h = torch.split(h_concat, self.num_hidden, dim=1)

        z_t = torch.sigmoid(z_x + z_h)
        r_t = torch.sigmoid(r_x + r_h)
        
        h_bar = torch.tanh(o_x + r_t * h_h)
        h_new = (1 - z_t) * h_t + z_t * h_bar
