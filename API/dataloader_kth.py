...
#from API.utils import AddPepperNoise,AddGaussianNoise
import logging
import os
import random
import cv2
...
class KTHDataset(Dataset):
    def __init__(self, train, datas, indices, pre_seq_length, aft_seq_length, require_back=False):
        ...
        self.std = 1
        self.train = train
        #self.mask =  np.load('/home/cjj/Documents/SimVPv2-master/data/kth/kth_mask.npy')
...
class DataProcess(object):
  ...
  def get_train_input_handle(self):
    if not os.path.exists(self.paths+'KTH/kth/'+'train_data_gzip.hkl'):
      ...
      train_data = hkl.load(self.paths+'KTH/kth/'+'train_data_gzip.hkl')
      train_indices = hkl.load(self.paths+'KTH/kth/'+'train_indices_gzip.hkl')
    return InputHandle(train_data, train_indices, self.input_param)

  def get_test_input_handle(self):
    if not os.path.exists(self.paths+'KTH/kth/'+'test_data_gzip.hkl'):
      ...
      test_data = hkl.load(self.paths+'KTH/kth/'+'test_data_gzip.hkl')
      test_indices = hkl.load(self.paths+'KTH/kth/'+'test_indices_gzip.hkl')
