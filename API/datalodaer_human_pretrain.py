...
import numpy as np
import torch
from API.utils import transforms_video

class Human_Occulusion_Train(Dataset):
    def __init__(self, data_dir, nt_cond, nt_pred):
    ...
    def __getitem__(self, index):
        ...
        output = self.data[idx_id:idx_id+self.lb+self.pred_h]
        output = transforms_video(output)
        return torch.tensor(output.transpose(0,3,1,2),dtype=torch.float)#torch.tensor(inputs.transpose(0,3,1,2),dtype=torch.float), torch.tensor(targets.transpose(0,3,1,2),dtype=torch.float)
...
class Human_Occulusion_Test(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #targets = self.data[idx_id+self.lb+1:idx_id+self.lb+1+self.pred_h]
        output = self.data[idx_id:idx_id+self.lb+1+self.pred_h]
        # for i in range(4):
...
def load_data(
    ...
    train_set = Human_Occulusion_Train(data_dir=data_root, nt_cond=4, nt_pred=4)
    test_set = Human_Occulusion_Test(data_dir=data_root, nt_cond=4, nt_pred=4)
