...
class KITTIDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        predict = self.preprocess(self.X[(begin+10):(end+1),:,:,:])
...
def load_data(
    ...
    train_set = KITTIDataset(data_file=dataset_train, source_file=source_train, batch_size=batch_size, shuffle=False, nt=11,sequence_start_mode='unique')
...
#   x = np.load('npy_kitti.npy')
#   print('npy load success!')
    # hkl.dump([1,2,3], 'foo.hkl')
    # hkl.load('foo.hkl')
    

    # x = np.load('dataset_train.npy')
    # hkl.dump(x,'X_train.hkl',mode='w')
    # x = hkl.load('X_train.hkl')
    # #print('stop')
    # x = np.load('dataset_train_source.npy')
    # hkl.dump(x,'sources_train.hkl',mode='w')
    # x = np.load('dataset_test.npy')
    # hkl.dump(x,'X_test.hkl',mode='w')
    # x = np.load('dataset_test_source.npy')
    # hkl.dump(x,'sources_test.hkl',mode='w')
    # x = np.load('dataset_val.npy')
    # hkl.dump(x,'X_val.hkl',mode='w')
    # x = np.load('dataset_val_source.npy')
    # hkl.dump(x,'sources_val.hkl',mode='w')
    # print('sucess!')
    #dataset_test_mini = os.path.join(data_root,'kitti/X_test_mini.hkl')
    #source_test_mini = os.path.join(data_root, 'kitti/sources_test_mini.hkl')
    # dataset_train = os.path.join(data_root,'kitti/X_train.hkl')
    # source_train = os.path.join(data_root, 'kitti/sources_train.hkl')
    # dataset_test = os.path.join(data_root, 'kitti/X_test.hkl')
    # source_test = os.path.join(data_root, 'kitti/sources_test.hkl')
    # dataset_valid = os.path.join(data_root, 'kitti/X_val.hkl')
    # source_valid = os.path.join(data_root, 'kitti/sources_val.hkl')
    # # x = hkl.load(dataset_train)
    # # np.save('dataset_train.npy',x)
    # # x = hkl.load(source_train)
    # # np.save('dataset_train_source.npy',x)
    # # x = hkl.load(dataset_test)
    # # np.save('dataset_test.npy',x)
    # # x = hkl.load(source_test)
    # # np.save('dataset_test_source.npy',x)
    # x = hkl.load(dataset_valid)
    # np.save('dataset_val.npy',x)
    # x = hkl.load(source_valid)
    # np.save('dataset_val_source.npy',x)
